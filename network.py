...
def build_point_pillar_graph(params: Parameters, batch_size: int = Parameters.batch_size):
    ...
    # pillars
    x = DOConv2D(nb_channels, (1, 1), activation='linear', use_bias=False, name="pillars/conv2d")(input_pillars)
    ...
    for n in range(4):
        ...
        x = DOConv2D(nb_channels, (3, 3), strides=S, padding="same", activation="linear",
    ...
    for n in range(6):
        S = (2, 2) if n == 0 else (1, 1)
        x = DOConv2D(2 * nb_channels, (3, 3), strides=S, padding="same", activation="linear",
    ...
    # Detection head
    occ = DOConv2D(nb_anchors, (1, 1), name="occupancy/conv2d", activation="sigmoid")(concat)

    loc = DOConv2D(nb_anchors * 3, (1, 1), name="loc/conv2d", kernel_initializer=tf.keras.initializers.TruncatedNormal(0, 0.001))(concat)
    loc = tf.keras.layers.Reshape(tuple(i//2 for i in image_size) + (nb_anchors, 3), name="loc/reshape")(loc)

    size = DOConv2D(nb_anchors * 3, (1, 1), name="size/conv2d", kernel_initializer=tf.keras.initializers.TruncatedNormal(0, 0.001))(concat)
    size = tf.keras.layers.Reshape(tuple(i//2 for i in image_size) + (nb_anchors, 3), name="size/reshape")(size)

    angle = DOConv2D(nb_anchors, (1, 1), name="angle/conv2d")(concat)

    heading = DOConv2D(nb_anchors, (1, 1), name="heading/conv2d", activation="sigmoid")(concat)

    clf = DOConv2D(nb_anchors * nb_classes, (1, 1), name="clf/conv2d", activation="linear")(concat)
...
def build_point_pillar_graph_depthwise_overparameterization(params: Parameters, batch_size: int = Parameters.batch_size):
